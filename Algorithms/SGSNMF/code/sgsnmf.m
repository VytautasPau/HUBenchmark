    ...
    for iter=1:para.maxiter 
      % stopping condition
       ...
       %  Update Wp
        ...
        Wpmatrix = 1./(double(para.M).^double(2).*Wpmatrix + 1);
...
% maxiter: limit of iterations
    ...
    alpha = 1; beta = 0.1;
    for iter=1:maxiter
        WpH = repmat(wp,1,np).*H; 
        WpH = WpH ./ repmat(sqrt(sum((WpH).^2)),M,1);
        grad = WtW*H - WtV + WpC .* WpH ;
        projgrad = norm(grad(grad < 0 | H >0));
        ...
        for inner_iter=1:20 
            ...
            gradd = grad.*d; 
            dQd = (WtW*d).*d;
            suff_decr = 0.99*sum(gradd(:)) + 0.5*sum(dQd(:)) < 0;
            if inner_iter==1, decr_alpha = ~suff_decr; Hp = H; end
            if decr_alpha  
                if suff_decr, H = Hn; break;
                else
